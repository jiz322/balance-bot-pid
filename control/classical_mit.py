...
from imu_driver_i2c import IMUDriverI2c
...
def main():
    ...
    if not imu.open():
        print("[Main] IMU I2C open failed, aborting.")
        return
    ...
    # Low-pass filters
    pitch_filter      = LowPassFilter(alpha=0.4)
    pitch_rate_filter = LowPassFilter(alpha=0.4)
    vel_filter        = LowPassFilter(alpha=0.2)
    yaw_rate_filter   = LowPassFilter(alpha=0.3)

    # Torque rate limiter (Nm/step)
    MAX_TORQUE_RATE = 2.0 * DT   # 2.0 Nm/s → 0.02 Nm per step
    prev_tff_l = 0.0
    prev_tff_r = 0.0
    ...
    try:
        while not shutdown:
            ...
            pitch_offset = clamp(pitch_offset, -0.10, 0.10)  # ~5.7° max
            ...
            if step % args.print_every == 0:
                ...
                print(f"  IMU rate:    {imu.get_actual_hz():.1f} Hz")
                print(f"{'='*65}")
